...
public interface MutableSortedNaturalOrderTestCase extends SortedNaturalOrderTestCase, MutableOrderedIterableTestCase, MutableCollectionTestCase
{
    ...
    default void MutableCollection_removeIf()
    {
        ...
        Assertions.assertFalse(collection2.removeIf(Predicates.equal(5)));
        Assertions.assertTrue(collection2.removeIf(Predicates.greaterThan(0)));
        Assertions.assertEquals(this.newWith(), collection2);
        Assertions.assertFalse(collection2.removeIf(Predicates.greaterThan(2)));
    ...
    default void MutableCollection_removeIfWith()
    {
        ...
        Assertions.assertTrue(collection.removeIfWith(Predicates2.<Integer>in(), Lists.immutable.with(5, 3, 1)));
        ...
        MutableCollection<Integer> collection2 = this.newWith(1, 2, 3, 4);
        Assertions.assertFalse(collection2.removeIfWith(Predicates2.equal(), 5));
        Assertions.assertTrue(collection2.removeIfWith(Predicates2.greaterThan(), 0));
        Assertions.assertEquals(this.newWith(), collection2);
        Assertions.assertFalse(collection2.removeIfWith(Predicates2.greaterThan(), 2));
